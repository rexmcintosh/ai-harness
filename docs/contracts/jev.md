...
| `council/signals.py` through `council/jev.py` (`council review`, the council step of `backlog-run`, `council sweep`) | three label-and-score signals after the chair has answered: verdict label, same problem from two seats, source of a chair block. Display and log only | shadow (branch `claude/council-jev-shadow`); own pin `jev-1.13.0`, own switch `COUNCIL_JEV=0`, scope is an allow list of repositories (`IN_SCOPE_REPOS`). `docs/council-jev-shadow-2026-09-19.md` |
| `tools/jev_council/` | offline council experiments | on main; imports the pin, the key loader, redaction and the scope rule from `council/jev.py` (before the council shadow signals: the names `watchdog/jev_shadow.py` keeps for it). Its own `ask` still hands the shared `http_post` to a retry loop and skips the usage ledger; should move to `import jev` (`jev.ask`) |
