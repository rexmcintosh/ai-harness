...
**Inventory date:** 2026-09-05, refreshed 2026-09-09 and 2026-09-18. **Contract format:** v1.0. These documents record observed configuration and intended operating boundaries. They do not change, validate, or enforce a cron job, timer, or external service.

The 2026-09-05 inventory came from `crontab -l` and `systemctl --user list-timers --all`. The 2026-09-09 refresh could not execute either command (the unattended runner denied both), so it used the newest crontab backup that session-gc wrote, `~/projects/.session-gc/crontab.backup.20260907150315` (2026-09-07 15:03 UTC), plus the systemd unit files, enablement links, and timer stamps on disk. The refresh found two cron entries added after 2026-09-05 and one dormant user timer; all three now have a row below. The 2026-09-18 refresh ran both commands live on `mesh-vps`. It confirmed the 2026-09-07 snapshot apart from the repository rename to `ai-harness` (paths only) and found three changes: the MeetTrack discover line was resumed on 2026-09-18, two Romance Empire ops-loop entries were added, and the user timer `attain-work-queue.timer` is active. Each has a row below. See "Re-verifying this inventory" for the confirmation step. A lifecycle may have more than one trigger. A successful process exit proves only that the entrypoint exited successfully; the evidence named in its contract is needed for the stated outcome.
...
| active | hourly at :17 (resumed 2026-09-18) | `/home/dev/projects/splash_poller/discover_meets.py` | MeetTrack v2 discover | [meettrack-supply-engine.md](meettrack-supply-engine.md) (v1.1, discover run contract v1.0). No run evidence on the host yet. |
| paused | every 5 min; every 5 min | `/home/dev/projects/splash_poller/supervise_meets.py`; `/home/dev/projects/splash_poller/ingest_entries.py` | MeetTrack v2 supervise and ingest | Paused in crontab since 2026-08-15. Remote database state was not checked. No run contract is active. Retirement question: [meettrack-supply-engine.md](meettrack-supply-engine.md). |
...
| MeetTrack v2 discover | meettrack-supply-engine.md v1.1 (discover run contract v1.0) | Yes in principle: `meet_registry` holds every meet on the live root list. Not evidenced: the script logs nothing on success and the cron log is empty. | No. Repair: print one summary line per run. |
| MeetTrack v2 supervise and ingest | meettrack-supply-engine.md v1.1 (retirement question) | None while paused. | **Candidate.** Owner decides resume or remove; see the record. |
...
| Romance Empire Notion ops loop | romance-ops-loop.md v1.0 | Yes: one `tick ... errors 0` line every ten minutes, one `compose: ... telegram sent` line per day, and a Result with evidence on every acted row. | No. Move the contract to the owning repository; no independent escalation when ticks stop (the watchdog only shadows its logs, without alerting). |
...
```
crontab -l | grep -vE '^\s*#|^\s*$|^[A-Za-z_]+=' | wc -l      # active entries: expect 28
crontab -l | grep -cE '^#\s*[0-9*]+/?[0-9]* '                  # commented job lines: expect 2
crontab -l | grep -vE '^\s*#|^\s*$|^[A-Za-z_]+=' | awk '{for(i=6;i<=NF;i++) printf "%s ", $i; print ""}' | sort
...
| Two stopped entries carry a pending owner decision. | Two MeetTrack lines (supervise, ingest) commented since 2026-08-15, while discover was resumed on 2026-09-18 before its contract existed; reaper timer not enabled since 2026-08-26. | Decision needed. Each document states the two options and the check that decides between them. No scheduler edit here. |
| The 2026-09-09 refresh was based on a two-day-old snapshot. | The runner denied `crontab -l` and `systemctl --user`. | Closed on 2026-09-18: both commands were run live; the differences found are the three changes listed at the top. |
| Loops keep appearing without a contract. | Between 2026-09-07 and 2026-09-18 two cron entries and one timer went live, and one paused loop was resumed, before any row existed here. | Process gap. The count check under "Re-verifying this inventory" makes the drift visible; nothing enforces contracts first. |
